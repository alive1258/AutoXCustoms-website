...
const CLIENT_VIDEO_REVIEW_URL = "/client-video-reviews";
...
export const clientVideoReviewApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    // 3. GET ACTIVE CLIENT VIDEO REVIEWS (Public homepage feed)
    getActiveClientVideoReviews: builder.query<
      ApiResponse<ClientVideoReviewItem[]>,
      void
    >({
      query: () => ({
        url: `${CLIENT_VIDEO_REVIEW_URL}/active`,
        method: "GET",
      }),
      providesTags: [tagTypes.client_video_reviews],
    }),
    ...
    >({
      query: (id) => ({
        url: `${CLIENT_VIDEO_REVIEW_URL}/${id}`,
        method: "GET",
      }),
      providesTags: [tagTypes.client_video_reviews],
    }),

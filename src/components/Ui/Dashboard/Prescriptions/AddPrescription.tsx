...
import { PrescriptionHistoryFields } from "@/src/types/historyType";
...
import { cmToFeetInches } from "./VitalsCalculatorModals";
...
const HISTORY_CATEGORY_LABELS: {
  key: keyof PrescriptionHistoryFields;
  label: string;
}[] = [
  ...
  { key: "social_history", label: "Social Habit" },
  { key: "surgical_history", label: "Past Surgical History" },
  { key: "sexual_history", label: "Sexual History" },
  { key: "travel_history", label: "Travel History" },
  { key: "oncologic_history", label: "Oncologic Therapies" },
  { key: "ot_notes", label: "OT Notes" },
];
...
const AddPrescription = () => {
  ...
  const [historyFields, setHistoryFields] = useState<PrescriptionHistoryFields>(
    {},
  );
  ...
  const onSubmit: SubmitHandler<AddPrescriptionFormValues> = async (values) => {
    try {
      await createPrescription({
        ...
        examination_findings: examinationFindings.length
          ? examinationFindings
          : undefined,
        ...historyFields,
        medicines,
  ...
  // Chief Complaints / On Examination don't have inline editors in the
  // document body — clicking them in the left nav should jump straight to
  // their modal instead of just highlighting an empty section.
  const handleLeftNavClick = (key: SectionKey) => {
    setActiveSection(key);
    if (key === "chief_complaints") setIsComplaintsModalOpen(true);
    if (key === "examination") setIsExaminationModalOpen(true);
    if (key === "history") setIsHistoryModalOpen(true);
  };
  ...
  return (
    <div className="rounded-lg border bg-white border-gray-200 overflow-hidden p-6">
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-[200px_1fr_220px] items-start">
          ...
          <div className="flex w-full min-h-[297mm] flex-col rounded-lg border border-gray-200 bg-white overflow-hidden">
            ...
            <div className="grid min-h-0 flex-1 grid-cols-4 grid-rows-[1fr] gap-3 p-5">
              <div className="col-span-1 flex flex-col gap-4 border-r-2 border-gray-200 pr-3">
                {/* Chief complaints and On examination summaries only render
                    once data exists — with nothing added yet there's nothing
                    to show here; use the left nav to open the entry modal. */}
                ...
                {examinationFindings.length > 0 && (
                  ...
                  >
                    ...
                    <ul className="flex flex-col gap-3">
                      {examinationFindings.map((item, index) => {
                        const rows: React.ReactNode[] = [];

                        if (item.height_cm) {
                          const { ft, inch } = cmToFeetInches(item.height_cm);
                          rows.push(
                            <li
                              key="height"
                          ...
                          rows.push(
                            ...
                            >
                              ...
                              <ul className="mt-0.5 flex flex-col gap-0.5 pl-4">
                                {item.bp_readings.map((reading, i) => (
                                  <li
                                    key={i}
                                    className="list-disc marker:text-gray-400"
                                  >
                        ...
                        if (item.temperature_readings?.length)
                          rows.push(
                            <li
                              key="temperature"
                              className="list-disc marker:text-gray-400"
                            >
                              Temperature:{" "}
                              {item.temperature_readings.join(", ")} °F
                            </li>,
                          );
                        if (item.pulse_readings?.length)
                          rows.push(
                            <li
                              key="pulse"
                              className="list-disc marker:text-gray-400"
                            >
                              Pulse: {item.pulse_readings.join(", ")} beats/min
                            </li>,
                          );
                        if (item.ibw)
                          rows.push(
                        ...
                        return (
                          ...
                          >
                            <button
                              ...
                              aria-label="Remove examination item"
                    ...
                    <button
                      type="button"
                      onClick={() => setIsExaminationModalOpen(true)}
                      className="mt-1 flex w-fit items-center gap-2 text-sm font-medium text-emerald-700 hover:text-emerald-800"
                    >
                      <Plus size={14} />
                      Add examination
                ...
                {HISTORY_CATEGORY_LABELS.some(
                  ({ key }) => (historyFields[key]?.length ?? 0) > 0,
                ) && (
                  <div
                    className={`flex flex-col gap-2 ${
                      chiefComplaints.length > 0 || examinationFindings.length > 0
                        ? "border-t border-gray-200 pt-4"
                        : ""
                    }`}
                  >
                    ...
                    <ul className="flex flex-col gap-1">
                      {HISTORY_CATEGORY_LABELS.filter(
                        ({ key }) => (historyFields[key]?.length ?? 0) > 0,
                      ).map(({ key, label }) => (
                        <li key={key} className="text-sm text-gray-700">
                          {label}{" "}
                          <span className="text-gray-400">
                            ({historyFields[key]?.length})
                          </span>
                        </li>
                      ))}
                    </ul>
      ...
      <OnExaminationModal
        ...
        initialItems={examinationFindings}
        patientGender={patientGender}
        onClose={() => setIsExaminationModalOpen(false)}
        onSave={setExaminationFindings}
      />

      <AddHistoryModal
        isOpen={isHistoryModalOpen}
        initialHistory={historyFields}
        onClose={() => setIsHistoryModalOpen(false)}
        onSave={setHistoryFields}
      />
    </div>
  );
};

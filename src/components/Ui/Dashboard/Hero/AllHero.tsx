...
import { Plus, Edit, Trash2, PanelTop } from "lucide-react";
  ...
  const handleDeleteHero = async (hero: HeroItem) => {
    try {
      const result = await Swal.fire({
        title: "Are you sure?",
        text: `Delete hero entry "${hero.title}"?`,
        icon: "warning",
      ...
      await Swal.fire({
        icon: "success",
        title: "Deleted!",
        text: `Hero entry "${hero.title}" has been deleted.`,
        timer: 1000,
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-6 border-b border-gray-200">
        <div>
          <h1 className="text-2xl font-semibold text-gray-200">Hero Section</h1>
